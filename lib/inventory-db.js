...
/**
 * Buscar item existente del mismo usuario con mismo nombre, tipo Y calidad (para merge)
 * Items con distinta calidad NO se unifican.
 */
export async function findExistingItem(userId, name, type, quality = 0) {
  const { data, error } = await supabase
    .from("inventory")
    .select("*")
    .eq("user_id", userId)
